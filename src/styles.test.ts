...
describe('responsive styles', () => {
  ...
  it('defines gradient appearance themes for rainbow candy and aurora', () => {
    ...
    expect(styles).toContain('rgba(255, 90, 103, 0.26)');
    expect(styles).toContain('rgba(85, 167, 255, 0.22)');
    expect(styles).toContain('rgba(155, 110, 243, 0.16)');
    expect(styles).toContain('rgba(67, 201, 120, 0.22)');
    expect(styles).toContain('rgba(67, 214, 198, 0.32)');
    expect(styles).toContain('rgba(66, 185, 232, 0.28)');
    ...
    expect(styles).toMatch(/@keyframes theme-glow-breathe\s*\{[\s\S]*40%,\s*\n\s*75%\s*\{[\s\S]*opacity:\s*var\(--theme-glow-opacity-max\);[\s\S]*transform:\s*scale\(var\(--theme-glow-scale\)\);/);
