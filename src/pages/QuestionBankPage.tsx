...
export default function QuestionBankPage() {
  ...
  function handleExportCsvTemplate() {
    ...
    void exportCsvTemplate()
      .then((result) => {
        ...
        setExportCsvMessage({
          kind: 'success',
          text: result === 'saved' ? 'CSV 範本已儲存。' : 'CSV 範本已下載至瀏覽器預設下載資料夾。',
        });
      })
      .catch((unknownError) => {
        ...
        setExportCsvMessage({ kind: 'error', text: 'CSV 範本儲存失敗，請重新嘗試。' });
      });
  }
  ...
  async function handleDownloadBackup() {
    ...
    try {
      ...
      setBackupModalMessage('備份已完成，請至瀏覽器下載資料夾查看。題庫未包含在備份檔中。');
      setBackupModalMessageType('success');
  ...
  async function handleRestoreFile(file: File | undefined) {
    ...
    try {
      ...
      if (!validation.isValid || !validation.backup) {
        setStatusMessage(validation.errors[0] ?? '無法還原：這不是有效的 JLS 備份檔。');
        return;
      }
      ...
      setStatusMessage(unknownError instanceof Error ? unknownError.message : '無法還原：這不是有效的 JLS 備份檔。');
    } finally {
  ...
  async function handleConfirmRestore() {
    ...
    try {
      ...
      setStatusMessage('還原完成，題庫未受到影響。');
      setActiveModal(null);
      window.setTimeout(() => window.location.reload(), 600);
    } catch (unknownError) {
      setStatusMessage(unknownError instanceof Error ? unknownError.message : '還原失敗，已保留原本資料。');
    } finally {
      setIsBusy(false);
    }
  }
    ...
    <section className="question-bank-page">
      ...
      <section className="library-actions" aria-label="Library actions">
        ...
        />
          ...
          >
            <span aria-hidden="true">📤</span>
            {exportCsvStatus === 'processing' ? '正在儲存…' : '匯出 CSV 範本'}
          </button>
      </section>
      ...
      <section className="today-card">
        ...
        <h2>{state.source === 'imported' ? '使用者匯入題庫' : '預設題庫'}</h2>
        <p>
      ...
      <details className="disclosure-card">
        ...
        <div className="knowledge-summary">
          <div>
            <span>題目總數</span>
            <strong>{state.validation.summary.totalQuestions}</strong>
          </div>
          <div>
            <span>年度</span>
            <strong>{state.validation.summary.yearCount}</strong>
          </div>
          <div>
            <span>科目</span>
            <strong>{state.validation.summary.subjectCount}</strong>
          </div>
          <div>
            <span>學習主題</span>
            <strong>{state.validation.summary.learningThemeCount}</strong>
          </div>
          <div>
            <span>核心概念</span>
            <strong>{state.validation.summary.knowledgeNodeCount}</strong>
          </div>
          <div>
            <span>選擇題</span>
            <strong>{state.validation.summary.choiceQuestionCount}</strong>
          </div>
          <div>
            <span>非選題</span>
            <strong>{state.validation.summary.essayQuestionCount}</strong>
          </div>
        </div>
      </details>
          ...
          <div className="modal-form">
            <label className="form-field">
              ...
              <select
                ...
                onChange={(event) =>
                  setWrongQuestionFilters({ year: event.target.value, subject: ALL, learningTheme: ALL })
                }
              >
                {wrongQuestionFilterOptions.years.map((year) => (
            ...
            <label className="form-field">
              ...
              <select
                ...
                onChange={(event) =>
                  setWrongQuestionFilters((current) => ({
                    ...current,
              ...
              >
                {wrongQuestionFilterOptions.subjects.map((subject) => (
            ...
            <label className="form-field">
              ...
              <select
                ...
                onChange={(event) =>
                  setWrongQuestionFilters((current) => ({ ...current, learningTheme: event.target.value }))
                }
              >
                {wrongQuestionFilterOptions.learningThemes.map((theme) => (
            ...
            <div className="modal-actions">
              {wrongQuestionExportStatus === 'success' ? null : (
                <button className="secondary-button" disabled={isBusy} type="button" onClick={closeModal}>
                  取消
                </button>
              )}
              {wrongQuestionExportStatus === 'success' ? (
                ...
                <button
                  ...
                  disabled={isBusy || wrongQuestionItems.length === 0}
                  type="button"
      ...
      {activeModal === 'backup' ? (
        <Modal isBusy={isBusy} title="備份" onClose={closeModal}>
          <div className="modal-form">
            <p>備份將保存作答紀錄、熟悉度、錯誤次數、學習統計、學習紀錄及系統基本設定，不包含題庫。</p>
            {backupModalMessage ? (
              <p className={backupModalMessageType === 'success' ? 'form-status' : 'form-error'}>{backupModalMessage}</p>
            ) : null}
            <div className="modal-actions">
              {backupStatus === 'success' ? null : (
                <button className="secondary-button" disabled={isBusy} type="button" onClick={closeModal}>
                  取消
                </button>
              )}
              {backupStatus === 'success' ? (
                ...
                <button className="primary-button" disabled={isBusy} type="button" onClick={() => void handleDownloadBackup()}>
                  {backupStatus === 'processing' ? '處理中…' : '下載備份'}
                </button>
              )}
            </div>
          </div>
        ...
        <Modal isBusy={isBusy} title="還原" onClose={closeModal}>
          <div className="modal-form">
            <dl className="restore-preview">
              <div>
                <dt>備份日期</dt>
                <dd>{formatDateTime(restorePreview.createdAt)}</dd>
              </div>
              <div>
                <dt>備份版本</dt>
                <dd>{restorePreview.backupVersion}</dd>
              </div>
            </dl>
            <p className="form-error">目前的學習紀錄與系統設定將被備份內容取代，題庫不會受到影響。</p>
            <div className="modal-actions">
              <button className="secondary-button" disabled={isBusy} type="button" onClick={closeModal}>
                取消
              </button>
...
export function ValidationReport({ validation }: { validation: QuestionBankValidationResult }) {
  ...
  return (
    <details className="validation-report__details">
  ...
  return (
    <section>
      ...
      <ul className="question-bank-issues">
        {issues.map((issue) => (
          <li key={`${issue.level}-${issue.rowNumber ?? 'header'}-${issue.field}-${issue.message}`}>
